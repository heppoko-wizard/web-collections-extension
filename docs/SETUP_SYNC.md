...
## 📁 v1.1.0: ローカルフォルダ同期の設定 (推奨)

OneDriveやGoogle Drive、iCloud Driveなどのクラウド同期フォルダを使用する場合、最も簡単で確実な方法です。

1. **同期用フォルダの用意**:
    - PC内で常時クラウド同期されている場所に、新しいフォルダを作成します（例: `Documents/WebCollectionsSync`）。
    - このフォルダは、Google DriveやOneDriveのクライアントによって自動的に同期される必要があります。

2. **拡張機能での設定**:
    - 拡張機能のサイドパネルを開き、⚙️（設定）アイコンをクリックします。
    - 「📁 ローカルフォルダ同期」セクションを探します。
    - 「📁 フォルダを選択」ボタンをクリックし、先ほど作成したフォルダを選択します。
    - ブラウザが「ファイルの編集権限」を求めてきた場合は、「許可」を選択してください。

3. **使い始め方**:
    - 現在拡張機能にあるデータをクラウドに上げたい場合は、「⬆️ エクスポート」をクリックします。
    - 他のPCでデータを受け取りたい場合は、「⬇️ インポート」をクリックします。
        - ※ インポートすると現在のローカルデータは上書きされます。注意してください。

4. **注意点**:
    - ブラウザを再起動した後、初回アクセス時に「権限の確認」が表示されることがあります。これはブラウザのセキュリティ仕様です。再度「許可」を押してください。
